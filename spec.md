# Chain Dominoes — Product and Game Specification

**Document status:** design specification only; no implementation is included.  
**Game index:** 75  
**Genre:** Turn-based tile game  
...
Chain Dominoes is a game in which players take turns extending open chain ends with matching tiles; empty the hand or minimize remaining pips. Its signature setting is a café tabletop with tactile ceramic dominoes. The product should feel immediately understandable, responsive within one input, and polished enough that the board or playfield itself is the visual hero. Sessions should begin quickly, make the next useful action obvious without solving the game for the player, and end with a clear explanation of score and progress.
...
Take turns extending open chain ends with matching tiles; empty the hand or minimize remaining pips.
...
The subject is the active playfield at near-tabletop to room scale, framed so state changes occupy most of the screen. The scene is a café tabletop with tactile ceramic dominoes. Use an authored camera, original procedural geometry, restrained environmental storytelling, and a deterministic visual seed. The no-post-processing baseline must still communicate hierarchy, depth, selection, and state.
...
- Ship a browser distribution with `starhermit.txt` at its root, `name=Chain Dominoes`, and `launch=index.html`. Keep source files, secrets, design documents, and source maps outside the uploaded distribution.
